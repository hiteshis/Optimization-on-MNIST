...
def sgd_optimization_mnist(learning_rate=0.025, n_epochs=5,
    ...
    n_train_batch = train_set_x.get_value(borrow=True).shape[0] // batch_size
    n_train_batches = train_set_x.get_value(borrow=True).shape[0] // batch_size_test
    ...
    old_W = T.matrix('old_W')
    old_b = T.matrix('old_b')
    ...
    computations = open("computations.txt", "w")
    while (epoch < n_epochs):
        ...
        for minibatch_index in range(n_train_batch):
            ...
            avg_cost_SGD.write("\n")
            computations.write(str(k))
            computations.write("\n")

            # iteration number
            iter = (epoch - 1) * n_train_batch + minibatch_index
